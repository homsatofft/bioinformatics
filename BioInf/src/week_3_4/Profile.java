...
public class Profile {
	...
	public double[] distribution(String text) {
		double[] result = new double[text.length() - _k];
		for (int i = 0; i < text.length() - _k; i++) {
			String pattern = text.substring(i, i + _k);
			result[i] = probability(pattern);
		}
		return result;
	}
